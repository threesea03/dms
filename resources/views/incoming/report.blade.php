                ...
                <div class="card" style="margin-top: 20px; margin-bottom:20px">
                    ...
                    <div class="card-body">
                        ...
                        @csrf
                            <div class="row">
                                <div class="col-md-6">
                                    <a href="{{ url('report') }}" class="btn btn-sm" title="Generate Report" style="background-color: #365880; color:white; margin-top:30px; margin-left:70px">
                                ...
                                <div class="col-md-6">
                                    <div class="from-group row"> 
                                        <div class="col-sm-3">
                                            ...
                                            <input type="search" class="form-control input-sm" id="searchuser" name="searchuser" style="height: 31px; width:150px; margin-top:38px" placeholder="Search user" required/>
                                        </div>
                                        <div class="col-sm-3">
                                            <label for="date" class="col-form-label col-sm-4">From</label>
                                            <input type="date" class="form-control input-sm" id="from" name="from" style="height: 31px; width:150px" required/>
                                        </div>
                                        <div class="col-sm-3">
                                            <label for="date" class="col-form-label col-sm-2">To</label>
                                            <input type="date" class="form-control input-sm" id="to" name="to" style="height: 31px; width:150px" required/>
                                        </div>
                                        <div class="col-sm-3">
                                            <button type="submit" class="btn" name="search" title="Search" style="margin-top: 35px; height:30px; width:40px"><svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path fill="currentColor" d="m18.677 19.607l-5.715-5.716a6 6 0 0 1-7.719-9.133a6 6 0 0 1 9.134 7.718l5.715 5.716l-1.414 1.414l-.001.001ZM9.485 5a4 4 0 1 0 2.917 1.264l.605.6l-.682-.68l-.012-.012A3.972 3.972 0 0 0 9.485 5Z"/></svg></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table table-striped">
                                ...
                                @foreach($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->done_incoming}}</td>
                                        <td>{{ $item->pending_incoming}}</td>
                                        <td>{{ $item->outgoing_count}}</td>
                                        <td>{{ $item->done_outgoing}}</td>
                                        <td>{{ $item->pending_outgoing}}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
